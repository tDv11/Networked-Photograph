...
while True:
    ...
    if( A + B == 0  ) or ( last_A+last_B == A+B ):
        lights_81=random.randint(0, 99)
        lights_84=int(100-lights_81)
        '''
        tn.write(('#output,81,1,65\x0d\x0a').encode('ascii'))
        tn.write(('#output,84,1,35\x0d\x0a').encode('ascii'))
    '''
        tn.write(('#output,81,1,'+str(lights_81)+'65\x0d\x0a').encode('ascii'))
        tn.write(('#output,84,1,'+str(lights_84)+'\x0d\x0a').encode('ascii'))
        print ('lights 81 = '+str(lights_81))
        print('there was no move in the gallery, light is set to 65/35')
        time.sleep(random.randint(20, 35))
        tn.write(('#output,84,1,'+str(lights_81)+'\x0d\x0a').encode('ascii'))
        tn.write(('#output,81,1,'+str(lights_84)+'\x0d\x0a').encode('ascii'))
